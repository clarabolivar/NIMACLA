...
with st.expander("Carga de datos", expanded=True):
    ...
    if datos is not None:
        nombre_dataset_anterior = st.session_state.get("nombre_dataset")

        if nombre_dataset_anterior != nombre_dataset_actual:
            for clave in [
                "items",
                "transacciones",
                "datos_codificados",
    ...
    if "datos" in st.session_state:
        ...
        if st.session_state["mostrar_datos_originales"]:
            ...
            st.dataframe(
                st.session_state["datos"].head(num_filas),
    ...
    tipo_interpretacion_anterior = st.session_state.get("tipo_interpretacion")

    if (
        tipo_interpretacion_anterior is not None
        and tipo_interpretacion_anterior != tipo_interpretacion
    ):
        for clave in [
            "items",
            "transacciones",
...
if "datos_codificados" in st.session_state:
    ...
    with st.expander("5. Resumen analítico", expanded=False):
        ...
        if not evaluaciones_analisis:
            ...
            if origen_analisis == "reglas filtradas" and criterios_filtrado_guardados:
                filtros_texto = ", ".join(
